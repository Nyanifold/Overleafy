...
import {
  ...
  type ApplyOptions,
  type ApplyReport,
  type BindingApplyReport,
  type BindingPlan,
  type BindingStorePort,
  type UnbindApplyReport,
  type UnbindPlan,
  type ConflictAbortReport,
  type ConflictContinueReport,
  type ConflictReport,
...
class RandomOperationIds implements OperationIdPort {
  next(): string {
...
export class SyncService {
  constructor(
    ...
    private readonly operationIds: OperationIdPort = new RandomOperationIds(),
  ) {}

  async status(repositoryPath: string): Promise<OperationResult<StatusReport>> {
  ...
  async plan(
    ...
    options: PlanOptions,
  ): Promise<OperationResult<SyncPlan>> {
  ...
  async apply(
  ...
  ): Promise<OperationResult<ApplyReport>> {
    if (this.execution === undefined) {
      throw new OverleafyError(
        "INTERNAL",
        "Sync execution services are not configured.",
      );
    }
    const binding = await this.requireBinding(repositoryPath);
    const state = await this.execution.states.read(repositoryPath);
    if (state.activeOperation !== undefined) {
      throw new OverleafyError(
        ...
        `Operation ${state.activeOperation.operationId} is already active.`,
        {
          remediation:
            "Inspect, continue, or abort the active operation before applying another plan.",
        },
      );
    }

    const initialObservation = await this.repositories.inspect(
      repositoryPath,
      binding,
      {
        fetch: true,
    ...
    const lock = await this.execution.locks.acquire(
      repositoryRoot,
      operationId,
    );
    const mergeAction = initialPlan.actions.find(
      (action) => action.type === "merge",
    );
    const pushAction = initialPlan.actions.find(
    ...
    try {
      const lockedObservation = await this.repositories.inspect(
        repositoryRoot,
        binding,
        {
          fetch: true,
      ...
      const finalObservation = await this.repositories.inspect(
        repositoryRoot,
        binding,
        { fetch: true },
      );
      ...
      if (headOid === null || remoteOid === null || headOid !== remoteOid) {
        throw new OverleafyError(
          "REMOTE_MOVED",
          "Post-sync verification did not observe equal local and remote commits.",
          {
            remediation:
              "Create a new plan; do not repeat the previous apply request.",
  ...
  ): Promise<OperationResult<ProjectBinding>> {
    const resolvedRepositoryPath = await this.repositories.bind(
      repositoryPath,
      binding,
    );
  ...
  async planBinding(
    repositoryPath: string,
    binding: ProjectBinding,
  ): Promise<OperationResult<BindingPlan>> {
    ...
    if (
      ...
      throw new OverleafyError(
        "BINDING_INVALID",
        "The worktree is already bound to a different Overleaf project or configuration.",
        {
          remediation:
            "Inspect and remove the existing binding explicitly before replacing it.",
          details: {
    ...
    const observation = await this.repositories.inspect(
      repositoryPath,
      undefined,
      { fetch: false },
    );
  ...
  async applyBinding(
    ...
    binding: ProjectBinding,
    planId: string,
  ): Promise<OperationResult<BindingApplyReport>> {
    const planned = await this.planBinding(repositoryPath, binding);
    if (planned.data.planId !== planId) {
      throw new OverleafyError(
        "PLAN_STALE",
        "The repository or binding changed after the binding plan was created.",
        {
    ...
    try {
      ...
      if (lockedPlan.data.planId !== planId) {
        throw new OverleafyError(
          "PLAN_STALE",
          "The repository or binding changed while acquiring the lock.",
          { remediation: "Create and inspect a new binding plan." },
        );
      }
  ...
  async planUnbind(
    repositoryPath: string,
  ): Promise<OperationResult<UnbindPlan>> {
    const binding = await this.bindings.read(repositoryPath);
    if (binding === undefined) {
      throw new OverleafyError(
        "BINDING_INVALID",
        "This repository is not bound to any Overleaf project.",
        { remediation: "No unbind action is necessary." },
      );
    }
    const body = {
      schemaVersion: 1 as const,
      ...
      remoteName: binding.remoteName,
      actions:
        (["remove_remote", "delete_config"] as UnbindPlan["actions"]),
    };
    return this.result({
      ...body,
      planId: sha256(body),
  ...
  async applyUnbind(
    repositoryPath: string,
    planId: string,
  ): Promise<OperationResult<UnbindApplyReport>> {
    const planned = await this.planUnbind(repositoryPath);
    if (planned.data.planId !== planId) {
      throw new OverleafyError(
        "PLAN_STALE",
        "The repository binding changed after the unbind plan was created.",
        { remediation: "Create and inspect a new unbind plan." },
      );
    }
    ...
    const operationId = this.operationIds.next();
    const root = await this.repositories.inspect(
      repositoryPath,
      binding,
      { fetch: false },
    );
  ...
  async conflicts(
    repositoryPath: string,
  ): Promise<OperationResult<ConflictReport>> {
    ...
    return this.result({
      ...
      files: await this.repositories.listConflicts(
        observation.snapshot.repositoryPath,
      ),
    });
  }
  ...
  async resolveConflict(
  ...
  ): Promise<OperationResult<ConflictResolutionReport>> {
    ...
    try {
      ...
      if (lockedActive.operationId !== active.operationId) {
        throw new OverleafyError(
          "OPERATION_IN_PROGRESS",
          "The active conflict operation changed while acquiring the lock.",
        );
      }
  ...
  async continueConflict(
    repositoryPath: string,
    expectedOperationId?: string,
  ): Promise<OperationResult<ConflictContinueReport>> {
    const execution = this.requireExecution();
    const binding = await this.requireBinding(repositoryPath);
    const active = await this.requireConflictOperation(repositoryPath);
    this.validateOperationId(expectedOperationId, active.operationId);
    if (
      active.mergeTargetOid === undefined ||
      active.expectedRemoteOid === undefined
    ) {
      throw new OverleafyError(
        "OPERATION_IN_PROGRESS",
        "The active conflict predates recoverable sync state.",
        {
          remediation:
            "Abort this operation, then create and apply a new sync plan.",
        },
      );
    }
    const observation = await this.repositories.inspect(
    ...
    try {
      ...
      await this.repositories.continueConflict(
        root,
        binding,
        active.mergeTargetOid,
        active.expectedRemoteOid,
      );
      ...
      if (headOid === null || remoteOid === null || headOid !== remoteOid) {
        throw new OverleafyError(
          "REMOTE_MOVED",
          "Conflict continuation could not verify equal local and remote commits.",
          { remediation: "Inspect the active operation and retry continuation." },
        );
      }
  ...
  async abortConflict(
    repositoryPath: string,
    expectedOperationId?: string,
  ): Promise<OperationResult<ConflictAbortReport>> {
    const execution = this.requireExecution();
    const active = await this.requireConflictOperation(repositoryPath);
    this.validateOperationId(expectedOperationId, active.operationId);
    const binding = await this.bindings.read(repositoryPath);
  ...
  private async requireBinding(repositoryPath: string): Promise<ProjectBinding> {
    const binding = await this.bindings.read(repositoryPath);
    if (binding === undefined) {
      throw new OverleafyError(
        "BINDING_INVALID",
        "The repository is not bound to an Overleaf project.",
        {
  ...
  private async requireConflictOperation(
    repositoryPath: string,
  ): Promise<NonNullable<
    Awaited<ReturnType<StateStorePort["read"]>>["activeOperation"]
  >> {
    const execution = this.requireExecution();
    ...
    if (
      ...
      throw new OverleafyError(
        "OPERATION_IN_PROGRESS",
        "There is no active sync conflict to inspect or recover.",
      );
    }
  ...
  private validatePlan(
    ...
    if (actualPlan.planId !== expectedPlanId) {
      throw new OverleafyError(
        "PLAN_STALE",
        "The repository or remote changed after the plan was created.",
        {
    ...
    if (actualPlan.blockedBy !== undefined) {
      throw new OverleafyError(
        actualPlan.blockedBy.code,
        actualPlan.blockedBy.message,
        {
    ...
    if (actualPlan.requiresConfirmation && options.confirmation !== true) {
      throw new OverleafyError(
        "REMOTE_REWRITTEN",
        "This plan requires explicit confirmation.",
        {
          remediation:
            "Review backup and rewrite actions, then apply with confirmation=true.",
        },
      );
    }
  }
  ...
  private validateLocalBranch(
    ...
    if (actualBranch !== binding.localBranch) {
      throw new OverleafyError(
        "BINDING_INVALID",
        `The worktree is on '${actualBranch ?? "detached HEAD"}', not the bound branch '${binding.localBranch}'.`,
        {
          remediation:
            `Switch to '${binding.localBranch}' or create a separate binding for this worktree.`,
          details: {
  ...
  private validateOperationId(
    ...
    if (
      ...
      throw new OverleafyError(
        "PLAN_STALE",
        "The active conflict operation changed after it was inspected.",
        {
          remediation:
            "List conflicts again and use the returned operation ID.",
  ...
  private result<T>(
    data: T,
    status: OperationResult<T>["status"] = "ok",
  ): OperationResult<T> {
    return {
      schemaVersion: RESULT_SCHEMA_VERSION,
