...
function isDirty(observation: SyncObservation): boolean {
  const { worktree } = observation.snapshot;
  return (
    worktree.staged.length > 0 ||
...
function dirtyPaths(observation: SyncObservation): string[] {
  const { worktree } = observation.snapshot;
  return [
    ...new Set([
      ...worktree.staged,
...
function classify(observation: SyncObservation): SyncClassification {
...
export function planSync(
  ...
  let blockedBy: ErrorDetails | undefined;

  const checkpointIfNeeded = (): boolean => {
    ...
    if ((observation.snapshot.worktree.outOfScope?.length ?? 0) > 0) {
      blockedBy = blocked(
        "DIRTY_WORKTREE",
        "Tracked or staged changes exist outside the configured sync scope.",
        "Commit, restore, or update include/exclude configuration before syncing.",
      );
      return false;
    }
    if (options.dirtyPolicy === "fail") {
      blockedBy = blocked(
        ...
        "Re-run with dirtyPolicy=checkpoint and provide a commit message, or commit the changes manually.",
      );
      return false;
    }
    if (options.dirtyPolicy === "stash") {
      blockedBy = blocked(
        "DIRTY_WORKTREE",
        "Stash planning is not valid for a complete two-way sync.",
        "Use dirtyPolicy=checkpoint or make the worktree clean.",
      );
      return false;
    }
  ...
  switch (classification) {
    ...
    case "remote_rewritten":
      ...
      if (options.rewritePolicy === "fail") {
        blockedBy = blocked(
          "REMOTE_REWRITTEN",
          "The remote branch no longer descends from the last observed remote commit.",
          "Inspect the history, then choose rewritePolicy=remote or rewritePolicy=local explicitly.",
        );
      } else if (snapshot.remoteOid === null) {
        blockedBy = blocked(
          "REMOTE_REWRITTEN",
          "The rewritten remote branch has no commit.",
          "Inspect the remote branch before choosing a recovery policy.",
        );
    ...
    case "operation_in_progress":
      blockedBy = blocked(
        snapshot.worktree.conflicted.length > 0
          ? "CONFLICT"
          : "OPERATION_IN_PROGRESS",
        "The repository already has an operation in progress.",
        "Continue or abort the existing operation before planning another sync.",
      );
      break;
    case "unborn":
      if (observation.relationship === "local_unborn" && snapshot.remoteOid !== null) {
        ...
        blockedBy = blocked(
          "DIRTY_WORKTREE",
          "No local commits and no files to commit.",
          "Add files and commit them, or set dirtyPolicy=checkpoint with at least one tracked file.",
        );
      }
      break;
    case "invalid":
      blockedBy = blocked(
        "BINDING_INVALID",
        "The repository is not on a named local branch.",
        "Switch to the configured local branch before syncing.",
      );
      break;
  }
  ...
  if (direction === "pull") {
    const pullActions = actions.filter(
      (a) => a.type !== "push" && a.type !== "force_push_with_lease",
    );
    actions.length = 0;
    ...
    if (actions.length === 0 && classification !== "equal_clean" && !blockedBy) {
      blockedBy = blocked(
        "BINDING_INVALID",
        "No local changes to pull.",
        "The remote has no new commits to bring in.",
      );
    }
