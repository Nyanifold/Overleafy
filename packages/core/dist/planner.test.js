...
function observation(relationship, overrides = {}) {
    return {
...
test("does not treat ordinary divergence as a remote rewrite", () => {
    ...
    assert.deepEqual(plan.actions.map((action) => action.type), ["create_backup_ref", "merge", "push"]);
});
test("requires an explicit policy for remote history rewrite", () => {
    const plan = planSync(observation("diverged", {
...
test("plan id is deterministic and changes with preconditions", () => {
    ...
    const changed = planSync(observation("equal", {
        snapshot: {
            ...cleanSnapshot,
            headOid: "different",
        },
