...
test("reports dirty paths and changes the worktree fingerprint", async () => {
    ...
    assert.notEqual(clean.snapshot.worktreeFingerprint, dirty.snapshot.worktreeFingerprint);
});
test("ignores excluded untracked output but reports excluded tracked edits", async () => {
    ...
    assert.equal(ignored.snapshot.worktreeFingerprint, clean.snapshot.worktreeFingerprint);
...
test("rejects remote URL drift and credentials embedded in an existing remote", async () => {
    ...
    await assert.rejects(repository.inspect(root, binding, { fetch: false }), (error) => error instanceof Error &&
        "details" in error &&
        error.details.code ===
            "BINDING_INVALID");
    await git(root, [
    ...
    await assert.rejects(repository.bind(root, {
        ...binding,
