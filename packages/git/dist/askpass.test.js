...
function runAskpass(executable, prompt, env) {
...
test("passes credentials through environment and removes helper", async () => {
    ...
    await withGitAskpass(binding, credentials, async (env) => {
        ...
        assert.equal(await runAskpass(helperPath, "Username for HTTPS", env), "git");
        assert.equal(await runAskpass(helperPath, "Password for HTTPS", env), "secret-token");
    });
    await assert.rejects(access(helperPath));
});
test("does not request credentials for a file remote", async () => {
    let requested = false;
    await withGitAskpass({ ...binding, gitUrl: "file:///tmp/overleaf.git" }, {
        async getGitToken() {
            requested = true;
            return "secret-token";
        },
...
//# sourceMappingURL=askpass.test.js.map