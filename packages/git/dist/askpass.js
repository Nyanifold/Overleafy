...
`;
function usesHttpAuthentication(gitUrl) {
    try {
...
export async function withGitAskpass(binding, credentials, action) {
    ...
    const directory = await mkdtemp(path.join(os.tmpdir(), "overleafy-askpass-"));
