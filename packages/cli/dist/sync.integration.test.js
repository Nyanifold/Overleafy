...
import { FileBindingStore, FileRepositoryLock, FileStateStore, } from "@nyanifold/config";
import { OverleafyError, SyncService, } from "@nyanifold/core";
...
async function createFixture() {
    ...
    const service = new SyncService(new GitRepository(), new FileBindingStore(), {
...
test("applies a reviewed binding plan and treats repetition as a no-op", async () => {
    ...
    const repeated = await service.applyBinding(root, binding, repeatedPlan.data.planId);
...
test("checkpoints a dirty worktree and pushes with remote verification", async () => {
    ...
    assert.deepEqual(plan.data.actions.map((action) => action.type), ["checkpoint", "push"]);
    const result = await service.apply(root, plan.data.planId, options);
    assert.equal(result.data.headOid, result.data.remoteOid);
    assert.equal(await git(root, ["status", "--porcelain"]), "");
    assert.equal(await git(remote, ["show", "master:main.tex"]), "local update");
});
test("fast-forwards a clean local branch after a remote edit", async () => {
    ...
    const result = await service.apply(root, plan.data.planId, options);
    assert.equal(result.data.headOid, result.data.remoteOid);
    assert.equal(await readFile(path.join(root, "remote.tex"), "utf8"), "remote update\n");
});
test("rejects a stale plan before creating a commit", async () => {
    ...
    await assert.rejects(service.apply(root, plan.data.planId, options), (error) => error instanceof OverleafyError &&
...
test("refuses to sync a branch other than the bound local branch", async () => {
    ...
    await assert.rejects(service.plan(root, {
        dirtyPolicy: "fail",
        rewritePolicy: "fail",
    }), (error) => error instanceof OverleafyError &&
...
test("blocks a restored remote history by default and accepts it explicitly", async () => {
    ...
    const options = {
        dirtyPolicy: "fail",
        rewritePolicy: "remote",
        confirmation: true,
    };
    const accepted = await service.plan(root, options);
    assert.deepEqual(accepted.data.actions.map((action) => action.type), ["create_backup_ref", "reset_to_remote"]);
