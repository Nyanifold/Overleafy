...
import { FileBindingStore, FileRepositoryLock, FileSecretStore, FileStateStore, ProfileGitCredentials, } from "@nyanifold/config";
import { OverleafyError, SyncService, RESULT_SCHEMA_VERSION, } from "@nyanifold/core";
import { GitRepository, GitRunner } from "@nyanifold/git";
import { createProjectBinding, OverleafAuthService, } from "@nyanifold/overleaf";
import { randomUUID } from "node:crypto";
import { mkdir, mkdtemp, readdir, rm, writeFile } from "node:fs/promises";
import { formatPlan, formatStatus, writeError, writeResult, } from "./output.js";
import { promptSecret, readSecretFile } from "./secret-input.js";
function service() {
    const secrets = new FileSecretStore();
    const repository = new GitRepository(new GitRunner(), new ProfileGitCredentials(secrets));
...
export function createProgram() {
    const program = new Command()
        .name("overleafy")
        .description("Synchronize a local Git worktree with an Overleaf project.")
        .version("0.1.0")
    ...
    auth
        ...
        try {
            ...
            writeResult(result({ profile: options.profile, gitTokenStored: true }), json, (data) => `Stored Git token for profile '${data.profile}'.`);
        }
        catch (error) {
    ...
    auth
        ...
        .description("Validate and store a Cookie from an already authenticated browser session.")
        .option("--profile <name>", "credential profile", "default")
        .option("--web-url <url>", "Overleaf web base URL", "https://www.overleaf.com")
        .option("--cookie-file <path>", "read Cookie from a regular file with mode 0600")
        .action(async (options, command) => {
        const json = isJson(command);
        try {
            const cookie = options.cookieFile === undefined
                ? await promptSecret("Overleaf browser Cookie: ")
                : await readSecretFile(options.cookieFile);
            const session = await authService().importCookie(options.profile, options.webUrl, cookie);
            writeResult(result({
                profile: session.profile,
                webUrl: session.webUrl,
                sessionValidated: true,
            }), json, (data) => `Validated browser session for profile '${data.profile}'.`);
        }
        catch (error) {
            writeError(error, json);
            process.exitCode = 8;
    ...
    auth
        ...
        try {
            ...
            writeResult(result(status), json, (data) => [
    ...
    projects
        ...
        const json = isJson(command);
        try {
            const values = await authService().listProjects(options.profile, options.webUrl);
            writeResult(result({ profile: options.profile, projects: values }), json, (data) => data.projects.length === 0
                ? "No projects found."
                : data.projects
    ...
    conflicts
        ...
        try {
            ...
            writeResult(response, json, (data) => data.files.length === 0
                ? `Operation ${data.operationId}: no unresolved paths.`
                : data.files
                    .map((file) => `${file.path}\tstages=${file.stages.join(",")}`)
                    .join("\n"));
        }
        catch (error) {
            writeError(error, json);
    ...
    conflicts
        ...
        .addOption(new Option("--use <side>", "side to keep")
        .choices(["ours", "theirs"])
        .makeOptionMandatory())
        .option("--repo <path>", "repository path", process.cwd())
        .action(async (options, command) => {
        const json = isJson(command);
        try {
            const response = await service().resolveConflict(options.repo, options.path, options.use);
            writeResult(response, json, (data) => `Resolved '${data.path}' using ${data.resolution}; ${data.remaining.length} conflict(s) remain.`);
        }
        catch (error) {
            writeError(error, json);
            process.exitCode = 8;
    ...
    conflicts
        ...
        try {
            ...
            writeResult(response, json, (data) => `Continued ${data.operationId}; local and remote are ${data.headOid}.`);
        }
        catch (error) {
            writeError(error, json);
            process.exitCode =
    ...
    conflicts
        ...
        try {
            ...
            writeResult(response, json, (data) => `Aborted ${data.operationId}; HEAD is ${data.headOid ?? "unborn"}.`);
        }
        catch (error) {
            writeError(error, json);
            process.exitCode = 8;
    ...
    program
        ...
        .option("--repo <path>", "repository path", process.cwd())
        .addOption(new Option("--dirty-policy <policy>", "local dirty worktree policy")
        .choices(["fail", "checkpoint", "stash"])
        .default("fail"))
        .addOption(new Option("--rewrite-policy <policy>", "remote rewrite policy")
    ...
    program
        ...
        try {
            ...
            await runner.run(["config", "user.name", options.name], { cwd: options.repo });
            await runner.run(["config", "user.email", options.email], { cwd: options.repo });
            writeResult(result({ name: options.name, email: options.email }), json, (data) => `Set Git identity: ${data.name} <${data.email}>.`);
        }
        catch (error) {
            writeError(error, json);
            process.exitCode = 8;
    ...
    program
        .command("clone")
        .description("Initialize a new Git repository, bind it to an Overleaf project, and pull any existing content.")
        ...
        try {
            ...
            if (projectId === undefined) {
                throw new OverleafyError("BINDING_INVALID", "A project ID or URL is required.", { remediation: "Pass the project ID as the first argument or via --project." });
            }
            // Ensure target directory exists and is empty.
            await mkdir(options.dir, { recursive: true });
            const entries = await readdir(options.dir);
            if (entries.length > 0 && !entries.every((e) => e === ".git")) {
                throw new OverleafyError("BINDING_INVALID", `Target directory '${options.dir}' is not empty.`, {
                    remediation: "Use an empty directory for clone, or run bind in an existing repository.",
                });
            }
            ...
            await runner.run(["symbolic-ref", "HEAD", "refs/heads/main"], { cwd: options.dir, allowExitCodes: [0, 128] });
            // Bind.
            ...
            const applied = await repoService.applyBinding(options.dir, binding, plan.data.planId);
            writeResult(applied, json, (data) => `Cloned ${data.projectId} to '${options.dir}' (remote: ${data.remoteName}/${options.remoteBranch}).`);
            ...
            try {
                ...
                if (pullPlan.data.actions.length > 0) {
                    const result = await repoService.apply(options.dir, pullPlan.data.planId, { dirtyPolicy: "fail", rewritePolicy: "fail", confirmation: false, direction: "pull" });
                    if (result.data.appliedActions.length > 0) {
                        pulled = true;
                        process.stderr.write(`Pulled ${result.data.appliedActions.length} commit(s) from Overleaf.\n`);
                    }
                }
            }
            catch {
            ...
            if (!pulled) {
                try {
                    ...
                    const zip = await authService().downloadZip(options.profile, webUrl, applied.data.projectId);
    ...
    program
        .command("bind")
        .description("Bind the repository to an Overleaf project. Plans and applies automatically; use --plan-id to apply a pre-reviewed plan.")
        ...
        try {
            ...
            const localBranch = options.localBranch ?? current.data.snapshot.branch;
            if (localBranch === null) {
                throw new OverleafyError("BINDING_INVALID", "Cannot bind a detached HEAD without --local-branch.");
            }
            const binding = createProjectBinding({
            ...
            if (options.planId !== undefined) {
                ...
                const applied = await syncService.applyBinding(options.repo, binding, options.planId);
                writeResult(applied, json, (data) => `${data.alreadyBound ? "Already bound" : "Bound"} ${data.projectId} to remote '${data.remoteName}' (${data.remoteBranch}).`);
            }
            else if (options.planOnly) {
                // Review mode: only generate the plan, print planId for later apply
                const planned = await syncService.planBinding(options.repo, binding);
                writeResult(planned, json, (data) => [
                ...
                const planned = await syncService.planBinding(options.repo, binding);
                const applied = await syncService.applyBinding(options.repo, binding, planned.data.planId);
                writeResult(applied, json, (data) => `${data.alreadyBound ? "Already bound" : "Bound"} ${data.projectId} to remote '${data.remoteName}' (${data.remoteBranch}).`);
            }
        }
        catch (error) {
    ...
    program
        .command("unbind")
        .description("Remove the Overleaf binding from this repository. Plans and applies automatically; use --plan-id to apply a pre-reviewed plan.")
        .option("--repo <path>", "repository path", process.cwd())
        ...
        try {
            if (options.planId !== undefined) {
                const applied = await service().applyUnbind(options.repo, options.planId);
                writeResult(applied, json, (data) => `${data.remoteRemoved ? "Removed" : "No"} remote '${data.remoteName}'. ${data.configDeleted ? "Deleted" : "No"} binding config.`);
            }
            else if (options.planOnly) {
                ...
                const planned = await service().planUnbind(options.repo);
                const applied = await service().applyUnbind(options.repo, planned.data.planId);
                writeResult(applied, json, (data) => `${data.remoteRemoved ? "Removed" : "No"} remote '${data.remoteName}'. ${data.configDeleted ? "Deleted" : "No"} binding config.`);
            }
        }
        catch (error) {
    ...
    program
        ...
        try {
            ...
            writeResult(result, json, (data) => data.appliedActions.length === 0
                ? "Already up to date."
    ...
    program
        ...
        .default("fail"))
        .option("--message <message>", "checkpoint commit message")
        .option("--confirm-rewrite", "confirm the reviewed remote history rewrite actions")
        .action(async (options, command) => {
        const json = isJson(command);
        try {
            const svc = service();
            ...
            if (options.planId) {
                ...
                const result = await svc.apply(options.repo, options.planId, applyOptions);
                writeResult(result, json, (data) => `Synchronized ${data.headOid ?? "unborn"} (${data.appliedActions.join(", ") || "no changes"}).`);
            }
            else {
                // Direct sync: plan internally and apply immediately
                const plan = await svc.plan(options.repo, applyOptions);
                const result = await svc.apply(options.repo, plan.data.planId, { ...applyOptions, confirmation: plan.data.requiresConfirmation || applyOptions.confirmation });
                writeResult(result, json, (data) => `Synchronized ${data.headOid ?? "unborn"} (${data.appliedActions.join(", ") || "no changes"}).`);
            }
        }
        catch (error) {
            writeError(error, json);
            const code = error instanceof OverleafyError ? error.details.code : "INTERNAL";
            process.exitCode =
                code === "CONFLICT"
                    ? 6
