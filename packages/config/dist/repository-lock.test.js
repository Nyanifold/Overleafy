...
test("serializes repository operations and releases by ownership token", async () => {
    ...
    await assert.rejects(locks.acquire(root, "op_second"), (error) => error instanceof OverleafyError && error.details.code === "LOCKED");
    await first.release();
    const second = await locks.acquire(root, "op_second");
    await second.release();
});
//# sourceMappingURL=repository-lock.test.js.map