...
test("writes atomically and discovers a binding from a nested path", async () => {
    ...
    const text = await readFile(path.join(root, ".overleafy", "config.json"), "utf8");
...
test("maps malformed config to BINDING_INVALID", async () => {
    ...
    await import("node:fs/promises").then(({ writeFile }) => writeFile(path.join(directory, "config.json"), "{}"));
    const store = new FileBindingStore();
    await assert.rejects(store.read(root), (error) => error instanceof OverleafyError &&
