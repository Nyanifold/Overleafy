...
async function stateRoot(startPath: string): Promise<string> {
  ...
  while (true) {
    try {
      if (
        (
          await stat(
            path.join(current, ".overleafy", "config.json"),
          )
        ).isFile()
      ) {
        return current;
...
export class FileStateStore implements StateStorePort {
  async read(repositoryPath: string): Promise<SyncState> {
    ...
    try {
      ...
      return stateSchema.parse(raw) as SyncState;
    } catch (error) {
      ...
      throw new OverleafyError(
        "BINDING_INVALID",
        `Invalid sync state at ${statePath}.`,
        {
          remediation:
            "Run doctor before changing or removing the state file.",
          details: {
            statePath,
  ...
  async write(repositoryPath: string, state: SyncState): Promise<void> {
