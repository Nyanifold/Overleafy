...
export class GitRepository implements RepositoryPort {
  constructor(
    private readonly runner: GitRunner = new GitRunner(),
    private readonly credentials?: GitCredentialProviderPort,
  ) {}

  async inspect(
    repositoryPath: string,
    binding: ProjectBinding | undefined,
    options: { fetch: boolean; lastRemoteOid?: string },
  ...
  async bind(
    ...
    const current = await this.runner.run(
      ["remote", "get-url", binding.remoteName],
      {
        cwd: root,
        allowExitCodes: [0, 2],
      },
    ...
    if (current.exitCode === 0) {
      ...
      if (!urlsEqual(currentUrl, binding.gitUrl)) {
        throw new OverleafyError(
          "BINDING_INVALID",
          `Remote '${binding.remoteName}' already points to a different URL.`,
          {
            remediation:
              "Choose another remote name or update the existing remote explicitly.",
            details: {
      ...
      await this.runner.run(
        ["remote", "add", binding.remoteName, binding.gitUrl],
        { cwd: root },
      );
    }
  ...
  async unbind(repositoryPath: string, remoteName: string): Promise<void> {
    const root = await this.resolveRoot(repositoryPath);
    await this.runner.run(
      ["remote", "remove", remoteName],
      { cwd: root, allowExitCodes: [0, 128] },
    );
  }
    ...
    for (const action of plan.actions) {
      switch (action.type) {
        case "checkpoint":
          ...
          });
          {
            const staged = await this.runner.run(
              ["diff", "--cached", "--quiet"],
              { cwd: root, allowExitCodes: [0, 1] },
            );
            if (staged.exitCode === 1) {
              await this.runner.run(
                [...hooksConfig, "commit", "-m", action.message ?? "Checkpoint"],
                { cwd: root },
              );
            }
          }
        ...
        case "merge":
          try {
            await this.runner.run(
              [...hooksConfig, "merge", "--no-edit", action.oid],
              { cwd: root },
            );
          } catch (error) {
            ...
            if (parsePorcelainV2(status.stdout).conflicted.length > 0) {
              throw new OverleafyError(
                ...
                {
                  remediation:
                    "List and resolve conflicts, then continue the active operation.",
                  cause: error,
                },
              );
            }
  ...
  async listConflicts(repositoryPath: string): Promise<ConflictFile[]> {
    const root = await this.resolveRoot(repositoryPath);
    const result = await this.runner.run(["ls-files", "-u", "-z"], {
      cwd: root,
    });
    const files = new Map<string, Set<1 | 2 | 3>>();
    for (const record of result.stdout.split("\0")) {
      ...
      const stage = Number(match[1]) as 1 | 2 | 3;
      const filePath = match[2];
      if (filePath === undefined) {
        continue;
      }
      const stages = files.get(filePath) ?? new Set<1 | 2 | 3>();
      stages.add(stage);
  ...
  async resolveConflict(
    ...
    const root = await this.resolveRoot(repositoryPath);
    const conflict = (await this.listConflicts(root)).find(
      (file) => file.path === filePath,
    );
    if (conflict === undefined) {
      throw new OverleafyError(
        "CONFLICT",
        `Path '${filePath}' is not an unresolved conflict.`,
        { remediation: "List conflicts again and use an exact returned path." },
      );
    }
  ...
  async continueConflict(
    ...
    if (conflicts.length > 0) {
      throw new OverleafyError(
        "CONFLICT",
        `${conflicts.length} path(s) still have unresolved conflicts.`,
        { remediation: "Resolve every listed conflict before continuing." },
      );
    }

    const gitDir = (
      await this.runner.run(["rev-parse", "--absolute-git-dir"], { cwd: root })
    ).stdout.trim();
    ...
    if (operation === "merge") {
      ...
      await mkdir(emptyHooks, { recursive: true, mode: 0o700 });
      await this.runner.run(
        ["-c", `core.hooksPath=${emptyHooks}`, "commit", "--no-edit"],
        { cwd: root },
      );
    } else if (operation !== "none") {
      throw new OverleafyError(
        "OPERATION_IN_PROGRESS",
        `Cannot continue a sync conflict during Git ${operation}.`,
      );
    }
    ...
    if (
      ...
      throw new OverleafyError(
        "OPERATION_IN_PROGRESS",
        "HEAD no longer contains the merge target from the active sync.",
        {
          remediation:
            "Abort the active sync or restore its merge before continuing.",
        },
      );
    }

    await this.withAuthentication(binding, async (env) => {
      const actualRemoteOid = await this.readRemoteOid(
        root,
        binding,
        env,
      );
  ...
  async abortConflict(
    ...
    if (operation !== "none") {
      throw new OverleafyError(
        "OPERATION_IN_PROGRESS",
        `Cannot abort a sync conflict during Git ${operation}.`,
      );
    }
    ...
    if (backupOid === null) {
      throw new OverleafyError(
        "GIT_FAILED",
        "The local backup ref for this active operation is missing.",
        {
          remediation:
            "Inspect the repository manually before clearing sync state.",
          details: { backupRef },
        },
      );
    }
  ...
  private async resolveRoot(repositoryPath: string): Promise<string> {
    try {
      ...
      throw new OverleafyError(
        "REPO_NOT_FOUND",
        `Not a Git worktree: ${path.resolve(repositoryPath)}`,
        {
  ...
  private async fetch(root: string, binding: ProjectBinding): Promise<void> {
    await this.withAuthentication(binding, async (env) => {
      try {
        await this.runner.run(
          [
  ...
  private async optionalRevParse(
    root: string,
    revision: string,
  ): Promise<string | null> {
    const result = await this.runner.run(
  ...
  private async snapshot(
    ...
    const worktree = this.filterWorktree(
    ...
    const remoteOid =
      binding === undefined
        ? null
        : await this.optionalRevParse(
            root,
            `refs/remotes/${binding.remoteName}/${binding.remoteBranch}`,
          );
  ...
  private async operation(gitDir: string): Promise<GitOperation> {
  ...
  private filterWorktree(
    ...
    const partition = (paths: string[]): [string[], string[]] => {
  ...
  private async fingerprintWorktree(
    root: string,
    headOid: string | null,
    branch: string | null,
  ...
  private async createBackupRefs(
    root: string,
    plan: SyncPlan,
    operationId: string,
    ...
    if (
      ...
      await this.runner.run(
        ["update-ref", `${base}/local`, plan.preconditions.headOid],
        { cwd: root },
      );
    }
    if (
      (target === "remote" || target === "both") &&
      plan.preconditions.remoteOid !== null
    ) {
      await this.runner.run(
        ["update-ref", `${base}/remote`, plan.preconditions.remoteOid],
        { cwd: root },
      );
    }
  }

  private async verifyRemoteLease(
    root: string,
    binding: ProjectBinding,
  ...
  private remoteMoved(
    ...
    return new OverleafyError(
      "REMOTE_MOVED",
      "The Overleaf remote moved after the plan was created.",
      {
  ...
  private async ensureLocalExclude(root: string): Promise<void> {
    const rawExcludePath = (
      await this.runner.run(["rev-parse", "--git-path", "info/exclude"], {
        cwd: root,
      })
    ).stdout.trim();
    ...
    if (!lines.has(".overleafy/")) {
      ...
      await appendFile(
        excludePath,
        `${separator}# overleafy managed\n.overleafy/\n`,
        "utf8",
      );
    }
  }

  private async assertBoundRemote(
    root: string,
    binding: ProjectBinding,
  ): Promise<void> {
    const result = await this.runner.run(
      ["remote", "get-url", binding.remoteName],
      { cwd: root, allowExitCodes: [0, 2] },
    );
    if (result.exitCode !== 0) {
      throw new OverleafyError(
        "BINDING_INVALID",
        `Configured remote '${binding.remoteName}' does not exist.`,
        { remediation: "Run bind again after inspecting the repository." },
      );
    }
    ...
    if (!urlsEqual(currentUrl, binding.gitUrl)) {
      throw new OverleafyError(
        "BINDING_INVALID",
        `Configured remote '${binding.remoteName}' no longer matches the binding.`,
        {
          remediation:
            "Inspect the remote change and bind again only if it is intentional.",
          details: {
            remoteName: binding.remoteName,
            currentUrl: this.redactUrl(currentUrl),
          },
  ...
  private assertCredentialFreeUrl(raw: string, remoteName: string): void {
    try {
      const url = new URL(raw);
      if (url.username !== "" || url.password !== "") {
        throw new OverleafyError(
          "BINDING_INVALID",
          `Remote '${remoteName}' contains credentials in its URL.`,
          {
            remediation:
              "Remove URL credentials and use a credential profile instead.",
          },
        );
      }
    } catch (error) {
