...
export function extractProjectId(project: string): string {
  ...
  try {
    ...
    const candidate =
      projectIndex >= 0 ? parts[projectIndex + 1] : parts.at(-1);
  ...
  throw new OverleafyError(
    "BINDING_INVALID",
    "Project must be a hexadecimal project ID or an Overleaf project URL.",
    {
      remediation:
        "Use an ID from the Overleaf project URL, for example 0123456789abcdef01234567.",
    },
  );
}
...
export function createProjectBinding(
  ...
  const webUrl = normalizeBaseUrl(
    options.webUrl ?? "https://www.overleaf.com",
    "web URL",
  );
  const gitUrl = normalizeBaseUrl(
    options.gitUrl ?? `https://git.overleaf.com/${projectId}`,
    "Git URL",
  );
